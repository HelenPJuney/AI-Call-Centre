"""
backend/livekit/kafka/producer.py
──────────────────────────────────────────────────────────────────────────────
FastAPI-side AIOKafka producer.  Redis has been REMOVED.

Lifecycle:
    startup  → await get_producer().start()
    shutdown → await get_producer().stop()

Usage in ai_worker.py:
    from .kafka.producer import get_producer
    from .kafka.schemas  import CallRequest

    await get_producer().submit_call_request(req)

The producer is a singleton created once per process (not per request).
Thread-safety: AIOKafka producer is coroutine-safe; no extra lock needed.

Fallback (Kafka unavailable):
    If the broker is unreachable at startup the _fallback flag is set.
    submit_call_request() returns None, signalling the caller to spawn
    ai_worker_task directly — identical to the pre-Kafka behaviour.

Queue position:
    Queue position is derived from Kafka consumer lag (end_offset - current
    offset) inside the Scheduler.  The producer simply returns None here;
    ai_worker.py returns queue_position=null for immediate calls and the
    browser can poll /livekit/queue-status/{session_id} for lag-based ETA.
"""
