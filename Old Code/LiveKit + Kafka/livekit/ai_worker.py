...
Two responsibilities:
  1. FastAPI router  — GET /livekit/token
     Issues a browser JWT, submits a Kafka call_request for scheduling,
     and returns LiveKit URL + optional queue position.

     KAFKA PATH (default):
       Browser → /livekit/token → produce call_requests → Kafka Scheduler
       → Scheduler assigns to GPU node → Worker Service spawns ai_worker_task
       Browser waits in LiveKit room; receives call_start DataChannel message
       when the AI worker joins.

     FALLBACK PATH (Kafka unavailable / dev mode):
       Identical to original behaviour — ai_worker_task spawned directly.
