"""
backend/livekit/kafka/health.py
──────────────────────────────────────────────────────────────────────────────
Prometheus metrics definitions and a FastAPI router for /livekit/kafka/health.

Metrics are registered once at module import and updated by WorkerService
and Scheduler components.

Usage:
    # In app.py / ai_worker.py router file:
    from backend.livekit.kafka.health import kafka_health_router
    app.include_router(kafka_health_router)

    # In WorkerService:
    from backend.livekit.kafka.health import (
        metric_active_calls, metric_calls_started, ...
    )
    metric_active_calls.labels(node_id=NODE_ID).set(len(active_tasks))

Install:
    pip install prometheus-client

Degrades gracefully (NoOp metrics) if prometheus-client is not installed.
"""
...
metric_gpu_vram_used_mb = _metric(
    Gauge if _PROM_AVAILABLE else _NoOpMetric,
