"""
backend/livekit/kafka/worker_service.py
──────────────────────────────────────────────────────────────────────────────
GPU Worker Service — standalone asyncio process, one per GPU node.

Run as:
    python -m backend.livekit.kafka.worker_service

Responsibilities:
    • Consume assigned call_requests from this node's Kafka partition
    • Launch ai_worker_task (existing code — UNCHANGED)
    • Retry failed tasks up to WORKER_MAX_RETRY times with exponential backoff
    • Monitor GPU with pynvml and publish gpu_capacity every 5 s
    • Publish call_started / call_completed / call_failed lifecycle events
    • Publish worker_heartbeat every 10 s (dead-man's switch for Scheduler)
    • Graceful shutdown: drain active tasks before exit (60 s timeout)

The existing ai_worker_task coroutine and everything under backend/livekit/
(audio_source, livekit_session, session_manager, token_service) are UNCHANGED.

Node partition assignment:
    The Kafka partition this service listens on is determined by NODE_PARTITION
    env var (default 0). Set a unique partition per node:
        NODE_ID=gpu-node-1 NODE_PARTITION=0 python -m ...
        NODE_ID=gpu-node-2 NODE_PARTITION=1 python -m ...
"""
...
# +----------------------------------+
# | run()                            |
# | * launch consume + heartbeat     |
# +----------------------------------+
#     |
#     v
# +----------------------------------+
# | _consume_assignments()           |
# | * read assignments, spawn tasks  |
# +----------------------------------+
#     |
#     v
# +----------------------------------+
# | _run_worker_with_lifecycle()     |
# | * wrap ai_worker_task + events   |
# +----------------------------------+
#     |
#     |----> _publish_call_started()
#     |----> ai_worker_task()
#     |----> _publish_call_completed()
#           OR
#     |----> _publish_call_failed()
#     |
#     v
# +----------------------------------+
# | <GpuMonitor> -> _publish_once()  |
# | * publish gpu_capacity to Kafka  |
# +----------------------------------+
#     |
#     v
# +----------------------------------+
# | _drain_and_shutdown()            |
# | * wait active tasks, stop        |
# +----------------------------------+
#     |
#     v
# [ END ]
...
class WorkerService:
    ...
    async def start(self) -> None:
        ...
        # FIX 2: Assign to TOPIC_CALL_ASSIGNMENTS (not call_requests).
    ...
    async def _run_worker_with_lifecycle(self, req: CallRequest) -> None:
        """
        Wraps ai_worker_task with:
            1. call_started publication
            2. Retry logic (up to WORKER_MAX_RETRY attempts)
            3. call_completed / call_failed / DLQ publication
            4. Removal from active_tasks

        FIX 5: ai_worker_task is imported at module level, not inside this loop.
        FIX 6: After all retries are exhausted the message is published to
               TOPIC_CALL_DLQ so operations teams can inspect and replay.
        """
        ...
        for attempt in range(WORKER_MAX_RETRY):
            ...
            try:
                ...
                # FIX 5: ai_worker_task already imported at module top level
