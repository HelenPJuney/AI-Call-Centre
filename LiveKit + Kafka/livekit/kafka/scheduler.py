"""
backend/livekit/kafka/scheduler.py
──────────────────────────────────────────────────────────────────────────────
Call Scheduler Service — standalone asyncio process.  Redis REMOVED.

Run as:
    python -m backend.livekit.kafka.scheduler

Architecture — Kafka-only:
    ┌──────────────┐    call_requests     ┌────────────┐
    │  FastAPI GW  │ ─────────────────▶  │  Scheduler │
    └──────────────┘                      │            │
                                          │ in-memory  │
    ┌──────────────┐    gpu_capacity      │ node_reg   │
    │  WorkerSvc   │ ─────────────────▶  │            │
    │              │ ◀──────────────────  │            │
    │  heartbeat   │    call_assignments  │            │
    └──────────────┘                      └────────────┘

Wait queue:
    Kafka IS the queue.  When all GPU nodes are full the Scheduler pauses
    call_requests partitions via consumer.pause().  When capacity returns
    (call_completed / call_failed event) it resumes them and processes
    waiting messages immediately.  Max backlog is bounded by MAX_QUEUE_SIZE
    which triggers a logged warning if lag exceeds the threshold.

Leader election:
    Kafka consumer-group guarantees exactly-one partition assignment across
    replicas — no Redis lock needed.  Each Scheduler replica only receives
    messages for its assigned partitions, preventing double-scheduling.

Node recovery:
    The in-memory node_registry tracks active_calls per node.  Dead nodes
    (no heartbeat for > SCHEDULER_NODE_DEAD_TIMEOUT_SEC) are evicted from
    the registry; their in-flight sessions are already tracked in the
    worker_service via call_started and will produce call_failed eventually.

Queue position broadcast:
    Kafka consumer lag (end_offset - committed_offset) is used as the queue
    position estimate and broadcast to waiting browsers via LiveKit DataChannel.
"""
